...
def generate_colourmap(boundary_values: list):
    ...
    cmap = mcolors.LinearSegmentedColormap.from_list(
        "green_yellow_red_cmap", list(zip(colour_positions, colours, strict=True))
    )
...
def add_colourmap(
    ...
    if boundaries is None:
        raise ValueError(
            "CI band boundaries are required; read them from reference.sqlite"
        )
    y_labels, y_tick_values = boundaries
